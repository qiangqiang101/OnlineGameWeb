...
Partial Class Admin_Dashboard
    ...
    Private Sub Admin_Dashboard_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Using db As New DataClassesDataContext
            newMember.InnerText = (From m In db.TblMembers Where m.DateCreated >= todayStart AndAlso m.DateCreated <= todayEnd).Count

            Dim transactions = db.TblTransactions.Where(Function(x) x.Status <= 1)
            Dim transfers = db.TblTransfers.Where(Function(x) x.Status <= 1)
            pendingRequest.InnerText = transactions.Count + transfers.Count
        End Using
    ...
    Private Function SumTotal(startDate As Date, endDate As Date) As Single
        Try
            Using db As New DataClassesDataContext
                Return (From s In db.TblTransactions Where s.TransactionDate >= startDate AndAlso s.TransactionDate <= endDate).Where(Function(x) x.Status = 2).Sum(Function(x) x.Credit) - (From s In db.TblTransactions Where s.TransactionDate >= startDate AndAlso s.TransactionDate <= endDate).Where(Function(x) x.Status = 2).Sum(Function(x) x.Debit) - (From s In db.TblTransactions Where s.TransactionDate >= startDate AndAlso s.TransactionDate <= endDate).Where(Function(x) x.Status = 2).Sum(Function(x) x.Promotion)
            End Using
