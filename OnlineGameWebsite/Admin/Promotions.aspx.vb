...
Partial Class Admin_Promotions
    ...
    Private Sub Admin_Promotions_Load(sender As Object, e As EventArgs) Handles Me.Load
        Dim promo = db.TblPromotions.OrderByDescending(Function(x) x.DisplayIndex AndAlso x.PromoID)
        For Each p As TblPromotion In promo
            Dim hasImage As Boolean = p.PromoImage <> Nothing
            dataTable.AddTableItem(p.PromoID.ToString("00000"), p.PromoName.Trim, If(hasImage, Img("..\" & p.PromoImage.Trim, p.PromoImage.Trim), ""), p.DisplayIndex, p.PromoPercent.ToString("0.00"),
                                   p.MaxPayout.ToString("0.00"), PromotionTypeToString(p.PromoType), PromotionStatusToString(p.Status),
