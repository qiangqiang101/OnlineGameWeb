...
Partial Class Partners_Summary
    ...
    Private Sub Partners_Summary_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Try
            Using db As New DataClassesDataContext
                ...
                Dim memCount = (From m In db.TblMembers Where m.Affiliate = Session("code").ToString.Trim And m.DateCreated >= start AndAlso m.DateCreated <= [end]).Count
                ...
                For Each p In products
                    dataTable.AddPSummaryReportTableItem(p.ProductName.Trim, TotalMemberDepositToday(p.ProductID, start, [end]), SummaryToday(p.ProductID, start, [end], eSum.Credit), SummaryToday(p.ProductID, start, [end], eSum.Debit), SummaryToday(p.ProductID, start, [end], eSum.Promotion))
                Next
        ...
        Try
            Using db As New DataClassesDataContext
                Return (From m In db.TblMembers Where m.Affiliate = Session("code").ToString.Trim
                        From t In db.TblTransactions Where t.UserName = m.UserName And t.Status = 2 And t.TransType = type And t.TransactionDate >= startDate AndAlso t.TransactionDate <= endDate
                        Select t).Count
            End Using
        Catch ex As Exception
            Return 0
    ...
    Private Function TotalMemberDepositToday(pid As Integer, startDate As Date, endDate As Date) As Integer
        Try
            Using db As New DataClassesDataContext
                Return (From m In db.TblMembers Where m.Affiliate = Session("code").ToString.Trim
                        From t In db.TblTransactions Where t.UserName = m.UserName And t.Status = 2 And t.ProductID = pid And t.TransactionDate >= startDate AndAlso t.TransactionDate <= endDate
                        Select t).Count
            End Using
        Catch ex As Exception
            Return 0
        ...
        Try
            Using db As New DataClassesDataContext
                Select Case sum
                    Case eSum.Credit
                        Return (From m In db.TblMembers Where m.Affiliate = Session("code").ToString.Trim
                                From t In db.TblTransactions Where t.UserName = m.UserName And t.ProductID = pid And t.Status = 2 And t.TransactionDate >= startDate AndAlso t.TransactionDate <= endDate
                                Select t).Sum(Function(x) x.Credit)
                    Case eSum.Promotion
                        Return (From m In db.TblMembers Where m.Affiliate = Session("code").ToString.Trim
                                From t In db.TblTransactions Where t.UserName = m.UserName And t.ProductID = pid And t.Status = 2 And t.TransactionDate >= startDate AndAlso t.TransactionDate <= endDate
